...
export const LessonItem: React.FC<PropLessonItem> = ({ lesson, locale, onPlay, topicId, enrolled }) => {
    ...
    const handleVideo = () => {
        if (enrolled)
            setVideos(lesson, topicId)
    }
    return (
        <li className={"flex items-center justify-between rounded-md hover:bg-card py-1 px-2 cursor-pointer " + (isActive() ? "bg-[#FFFFFF10]" : "")} onClick={handleVideo}>
